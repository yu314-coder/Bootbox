...
(function () {
  ...
  const SYSTEMS = [
    { id: "x64pw", name: "64-bit Linux + Python & Wine", sub: "REAL 64-bit Alpine · Python 3.12 + pip + Wine · terminal + graphical output · internet", icon: "🐍", tag: "64-BIT",
      guest: "x64pw", ram: 512,
      note: "Real 64-bit x86_64 Alpine Linux via QEMU-Wasm — now DUAL-CORE (nproc = 2: two jobs really run at once, and a busy program no longer freezes the shell). Python 3.12 + pip + Wine 9.0 (BOTH 32-bit and 64-bit .exe) PRE-INSTALLED, with REAL internet. FAST pip: ~24 common packages pre-loaded (instant); big installs work (10-min download timeout); data stack: `pip install numpy` gives the compatible 1.x build automatically (numpy/scipy/pandas ship Alpine wheels; for scikit-learn use `apk add py3-scikit-learn`). mc file manager included. Terminal LEFT; GUI programs (X apps / Wine .exe) appear on the RIGHT. Downloaded once (~220 MB), ~2-3 min first cold-boot." },
