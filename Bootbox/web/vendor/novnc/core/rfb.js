...
export default class RFB extends EventTargetMixin {
    ...
    _sendEncodings() {
        ...
        // so this is NOT a bridge bug) reproduced it: any of Tight/ZRLE/JPEG or several pseudo-
        // encodings (cursor, desktop-size, extended-clipboard, …) makes Xvnc withhold all pixels,
        // while [Hextile, CopyRect, Raw, QEMUExtendedKeyEvent] reliably delivers complete updates.
        // Hextile is also the cheapest to encode, which suits the slow emulated CPU. The cursor is
        // rendered server-side into the framebuffer (no Cursor pseudo-encoding); the desktop is a
        // fixed 1024x768 (no DesktopSize). Keep this list minimal — re-adding the others brings the
        // black-screen hang straight back.
        encs.push(encodings.encodingCopyRect);
        encs.push(encodings.encodingHextile);
        encs.push(encodings.encodingRaw);
        encs.push(encodings.pseudoEncodingQEMUExtendedKeyEvent);
